...
BEGIN
 SELECT SALARY,
 ROUND(MONTHS_BETWEEN(SYSDATE,HIRE_DATE)/12,2) AS TOTAL_EXP INTO SAL,T_EXP
 FROM EMPLOYEES
 WHERE EMPLOYEE_ID=&EID;
 ...
 ELSE (SAL+(SAL*0.10)) END AS NEW_SAL INTO FNAME,LNAME,FINAL_SALARY
 FROM EMPLOYEES
 WHERE EMPLOYEE_ID=&EID;
 
 DBMS_OUTPUT.PUT_LINE(FNAME||' '||LNAME||' '||SAL||' '||FINAL_SALARY);
END;
...
BEGIN
 ...
 CASE
  WHEN T_EXP BETWEEN 1 AND 5 THEN (SAL+(SAL*0.20))
  WHEN T_EXP BETWEEN 5 AND 10 THEN (SAL+(SAL*0.15))
  ELSE (SAL+(SAL*0.10)) END AS NE_SAL INTO FNAME,LNAME,FI_SAL
  FROM EMPLOYEES
  WHERE EMPLOYEE_ID=&EID;
  
  DBMS_OUTPUT.PUT_LINE(FNAME||' '||LNAME||SAL||' '||FI_SAL);
  END;
